...
export default function Informasi() {
  const informasiList = [
    {
      id: 1,
      type: 'pengumuman',
      icon: <AlertCircle className="w-5 h-5 text-ppu-red" />,
    ...
    },
    {
      id: 2,
      type: 'jadwal',
      icon: <CalendarDays className="w-5 h-5 text-ppu-blue" />,
    ...
    },
    {
      id: 3,
      type: 'panduan',
      icon: <FileText className="w-5 h-5 text-ppu-blue" />,
  ...
  return (
    <div className="flex-1 flex flex-col items-center py-12 px-4 sm:px-6 lg:px-8 w-full max-w-4xl mx-auto">
      <div className="text-center mb-10 w-full animate-fade-in animate-in fade-in duration-500">
        ...
        <h1 className="text-3xl sm:text-4xl font-black text-ppu-blue tracking-tight uppercase mb-3">
          Informasi Pemilu
        </h1>
        <p className="text-slate-600 font-semibold sm:text-lg max-w-2xl mx-auto">
      ...
      <div className="w-full space-y-6">
        {informasiList.map((item) => (
          <div 
            key={item.id} 
            className="bg-white border border-ppu-border p-6 sm:p-8 rounded-2xl shadow-md hover:border-ppu-blue/30 transition-colors group relative overflow-hidden"
          >
            ...
            <div className="flex items-start gap-4 sm:gap-6">
              <div className="w-12 h-12 shrink-0 bg-slate-50 border border-ppu-border rounded-xl flex items-center justify-center shadow-sm">
                {item.icon}
              </div>
              <div className="flex-1">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-2">
                  <h3 className="text-lg sm:text-xl font-bold text-[#0B1220] capitalize leading-tight">
                    {item.title}
                  </h3>
                  <span className="text-xs font-semibold text-slate-500 bg-slate-100 border border-ppu-border px-3 py-1 rounded-full whitespace-nowrap self-start sm:self-center">
                    {item.date}
                  </span>
                </div>
                <div className="text-sm text-slate-600 leading-relaxed text-left space-y-2 whitespace-pre-line font-medium mt-3">
                  {item.content}
                </div>
              </div>
            </div>
